...
TODO
    */
    ...
    let left_vals: Vec<Fp> = v.iter().enumerate().filter(|&(i, _)| (i+1) % 2 == 0).map(|(_, e)| *e).collect();
    let right_vals: Vec<Fp>  = v.iter().enumerate().filter(|&(i, _)| i % 2 == 0).map(|(_, e)| *e).collect();
    let new_roots: Vec<Fp> = roots.iter().enumerate().filter(|&(i, _)| (i+1) % 2 == 0).map(|(_, e)| *e).collect();

    let left = _fft(&left_vals, &new_roots);
    let right = _fft(&right_vals, &new_roots); 
    let output: Vec<Fp> = vec![Fp::new(BigUint::from(0u32)); v.len()];
...
fn fft_inv(v: &Vec<Fp>, root_of_unity: &Fp) -> Vec<Fp> {
    ...
    while roots_of_unity[roots_of_unity.len()-1] != Fp::new(BigUint::from(1u32)) {
        roots_of_unity.push(roots_of_unity[roots_of_unity.len()-1] * root_of_unity.clone())
    }
...
fn verify_mimc_proof(inp: u32, num_steps: u32, round_constants: &[u32], num_round_constants: u32, output: u32/*, proof: Proof*/) -> bool {
    const modulus: BigUint = Fp::get_modulus();

    if num_steps > (2u32.pow(32u32) / EXTENSION_FACTOR) { //TODO use of floor here?
        return false;
    }

    if !is_power_of_2(num_steps) || !is_power_of_2(num_round_constants) {
        return false;
    }

    if num_round_constants < num_steps.into() {
        return false;
    }

    let precision = num_steps * EXTENSION_FACTOR;
    let G2 = Fp::new(BigUint::from(7u32).pow((modulus - 1) / precision)); // TODO do I need floor() here for some reason?
    let skips = precision / num_steps;
    let constants_mini_polynomial = fft_inv(round_constants, G2.pow(EXTENSION_FACTOR*skips));
